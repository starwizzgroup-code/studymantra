...
router.post('/request-email-update', Middleware, async (req, res) => {
    ...
    try {
        ...
        if (!id || !role || !newemail) return res.status(400).json({ message: 'Missing require data' })
        let user;
        if (role === 'user') {
...
router.patch('/verify-email-update', Middleware, async (req, res) => {
